...
# @Last modified time: 18-Nov-2016 18:11
#
...
outfolder = 'out1'
outfolder90 = '90_out1'
outfolder_color = 'simdatabase/f1'
if os.path.exists(aout):
    shutil.rmtree(aout)
if os.path.exists(outfolder):
    shutil.rmtree(outfolder)
if os.path.exists(outfolder90):
    shutil.rmtree(outfolder90)
if os.path.exists(outfolder_color):
    shutil.rmtree(outfolder_color)

# now create them
os.makedirs(aout)             # jediaverage will write fits files inside aout
os.makedirs(outfolder)        # jedicatalog will create folders inside out1
os.makedirs(outfolder90)      # jedicatalog will create folders inside 90_out1
os.makedirs(outfolder_color)  # jedicolor will write fitsfiles inside sim/f1


# ======================================================================
# parse config file
config_path = r'physics_settings/config1_original.conf'
...
def run_process(name, args,):
    """Run a given process and terminate if external process fails."""
    print("-------------------------------------------------")
    print("Running: %s\nCommand:" % name)
    for arg in args:
        print(arg, end=' ')
