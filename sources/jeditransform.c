 ...
 * Run         :
 * ./jeditransform out1/trial0_catalog.txt out1/trial0_dislist.txt
 *  executable     input_catalog_from_jedicatalog      output_dislist_for_jedidistort
 *
 *
 *
 * Depends     : 1. simdatabase/bulge_disk_f8/f8_bulge_disk_0_to_301.fits # input fitsfiles to transform.
 *               2. out1/trial0_catalog.txt # catalog to transform input fitsfiles
 *               3. out1/stamp_0/stamp_0_to12  # 13 empty output folders to transform above fitsfiles as zipped files
 *
 * Inputs      : 1. out1/trial0_catalog.txt
 *
 *
 * Outputs     : 1. 12,420 zipped stamps
 *                  e.g. out1/stamp_0/stamp_0.fits.gz  (stamp_0 to stamp_12)
 *               2. out1/trial0_dislist.txt
 *
 * Info        :
 * This program takes in the catalog list created by jedicatalog:
 * (e.g. out1/trial0_catalog.txt)
 *
 * reads the galaxies names which are to be transformed
 * (e.g. simdatabase/bulge_disk_f8/f8_bulge_disk0.fits
 *       
 * and also read other parameters needed to transform that galaxy from fitsfiles headers
 * (e.g. input_name x y angle redshift pixscale old_mag old_r50 new_mag new_r50 stamp1 stmap2)
 *
 ...
 * out1/stamp_0/stamp_0_to_999.fits.gz  (for stamps 0 to 12 )
 *
 * It also creates dislist.txt for the jedidistortion,viz.,
 * out1/trial0_dislist.txt
 * 
 * Row of distlist.txt from jeditransform for jedidistort:
 * 5838    1741   13       7        1.500000  out1/stamp_0/stamp_0.fits.gz  out1/distorted_0/distorted_0.fits
 * xembed  yembed x-width  y-width  redshift  out_stamp_from_jeditransform  fitsfile_from_jedidistort   
 * 
 ...
 * 
 * 
 * Estimated time : 8 min 30 sec for f8_bulge_disk 302 fitsfiles.
 *
 */
...
int main(int argc, char *argv[]){
    ...
    for(g = 0; g < ngalaxies; g++){
        ...
        fits_update_key(outfptr,   TFLOAT,    "REDSHIFT",   &galaxies[g].redshift,     "redshift of this galaxy.",                          &status); // May 8, 2017
        fits_update_key(outfptr,   TFLOAT,    "PIXSCALE",   &galaxies[g].pixscale,     "pixscale of this galaxy.",                          &status); // May 8, 2017
